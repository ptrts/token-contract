...
solFile=./contracts.sol
variablesJsFile=./variables.js
mainJsFile=./deploy.js
...
processContract Test

# launch geth
# attach to the node
# exec deployParametersJsFile
# exec deployJsFile
geth --exec "loadScript(\"${variablesJsFile}\"); loadScript(\"${mainJsFile}\")" attach ${ipcFile} | grep -P '^(var .*;|//.*)$' >> ${variablesJsFile}
